...
function test_setup_dsl_job() {
  EXPECTED_JOBS_ARRAY=("SetupDSLJobs" "NodeSetupPipeline" "GenerateCRUMPipeline" "PythonDependenciesVerification_CarelessVaquita"
...
docker run -d --name test_jenkins_instance --env-file .env_example --network jenkins_network "$DOCKERHUB_REPO"
